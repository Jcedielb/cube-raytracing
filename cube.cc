...
bool cube::hit(const ray &r, double t_min, double t_max, hit_record &rec) const {
    double t0 = t_min;
    double t1 = t_max;
    // Método de slabs: para cada eje se calcula el intervalo en el que el rayo interseca
    // las caras de la caja; si no hay un intervalo común, no hay impacto.
    for (int i = 0; i < 3; i++) {
    ...
    double bias = 1e-4;
    vec3 outward_normal;
    // Determina la normal basada en qué cara se golpea, usando un bias para compensar
...
bool hit_cube(const ray &r, double t_min, double t_max, hit_record &rec) {
    // Define un cubo unitario sin material (para funciones auxiliares como sombras)
